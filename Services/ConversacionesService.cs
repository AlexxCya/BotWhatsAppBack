...
namespace BotWhatsApp.Services
{
    public class ConversacionesService : IConversacionesService
    {
        ...
        public async Task InsertConversaciones(ConversacionesDTO conversacionesDTO, string uri)
        {
            ...
            GetChat(conversacionesDTO.Destinatario);
            

        }
        public Chat GetChat(string Destinatario)
        {

                Chat chat = new Chat();
                List<Message> messages = new List<Message>();
                var lstMsgs = _unitOfWork.ConversacionesRepository.GetAll().Where(x => x.Destinatario == Destinatario);
                var _bandeja = _bandejaService.GetBandejaPorDestinatario(Destinatario, 1);
        ...
        public List<Chat> GetChats(string Destinatario)
        {
            ...
            foreach (var item in lstDestinatarios)
            {
                Chat chat = new Chat();
                List<Message> messages = new List<Message>();
                var lstMsgs = _unitOfWork.ConversacionesRepository.GetAll().Where(x => x.Destinatario == item.Destinatario);
                var _bandeja = _bandejaService.GetBandejaPorDestinatario(item.Destinatario, 1);
        ...
        public Chat GetChatByDestinatario(string Destinatario)
        {
                Chat chat = new Chat();
                List<Message> messages = new List<Message>();
                var lstMsgs = _unitOfWork.ConversacionesRepository.GetAll().Where(x => x.Destinatario == Destinatario);
